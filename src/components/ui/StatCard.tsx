...
export function StatCard({ label, value, change, prefix, suffix, icon, iconColor = "text-rose-500", loading }: StatCardProps) {
  ...
  if (loading) {
    return (
      <div className="bg-card border border-border/60 rounded-[8px] p-5">
  ...
  return (
    <div className="bg-card border border-border/60 rounded-[8px] p-5 hover:border-border transition-colors">
      ...
      <div className="font-display font-bold text-2xl text-foreground tracking-tight">
        {prefix && <span className="text-muted-foreground text-lg mr-1">{prefix}</span>}
